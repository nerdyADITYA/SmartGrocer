...
public class ShopGroceryAdapter extends ArrayAdapter<GroceryModel> {
    ...
    public ShopGroceryAdapter(@NonNull Context cont, int source, List<GroceryModel> list, TextView tvTotalPrice, Button checkoutButton) {
...
//                //it stores number of grocery in cart.
//                GroceryModel gm = list.get(position);
//
//                if(qty>0) {
...
//
//                    //it stores the grocery in the cart.
//                    SharedPreferences shad = getContext().getSharedPreferences("Cart", Context.MODE_PRIVATE);
//                    SharedPreferences.Editor edit = shad.edit();
//
//                    shadcount = getContext().getSharedPreferences("Count", Context.MODE_PRIVATE);
//                    SharedPreferences.Editor editcount = shadcount.edit();
//
//                    gm.setQty(qty);
//                    Gson gson = new Gson();
...
//                    editcount.putInt("countme", count);
//                    edit.commit();
//                    editcount.commit();
//
//                    Toast.makeText(cont.getApplicationContext(), gm.getName() + " HAS BEEN ADDED TO CART : " + qty, Toast.LENGTH_SHORT).show();
//                }
//                else
//                {
//                    Toast.makeText(cont, "SELECT ITEM", Toast.LENGTH_SHORT).show();
//                }
            }
        });
        ...
        btSub.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (qty > 0) {
                    ...
                    itemName.set(position, gr.getName());
                    itemPrice.set(position, "" + gr.getPrice());
                    itemQty.set(position, "" + qty);

                    GroceryModel newGr = new GroceryModel(gr.getgId(), gr.getImgUrl(), gr.getName(), gr.getPrice(), gr.getQty(), gr.getUnit());

                    list.set(position, newGr);
